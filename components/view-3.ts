...
const [successData, setSuccessData] = useState<{ url: string; title: string } | null>(null);
  ...
  // Lista de mensagens para simular as etapas de geração do PDF
const progressSteps = [
  "Analisando a estrutura do seu livro...",
  "Montando a folha de rosto...",
  "Escrevendo a introdução...",
  "Formatando os capítulos iniciais...",
  "Revisando o fluxo de conteúdo...",
  "Diagramando os capítulos centrais...",
  "Ajustando a tipografia e o espaçamento...",
  "Criando o sumário...",
  "Compilando a conclusão...",
  "Aplicando a hifenização e justificação...",
  "Gerando a versão final do arquivo PDF...",
  "Quase pronto, aplicando os últimos retoques..."
];
...
// components/ViewBookPage.tsx
  ...
  async function handleGeneratePdf() {
    ...
    setError(null);
    setLog([]); // Limpa o log antigo
    
    // --== INÍCIO DO NOVO BLOCO DE PROGRESSO ==--
    let step = 0;
    setProgressMessage(progressSteps[step]); // Define a primeira mensagem imediatamente

    const progressInterval = setInterval(() => {
      // Avança para a próxima etapa, ou volta para o início se a lista acabar
      step = (step + 1) % progressSteps.length; 
      setProgressMessage(progressSteps[step]);
    }, 2500); // Muda a mensagem a cada 2.5 segundos
    // --== FIM DO NOVO BLOCO DE PROGRESSO ==--

    try {
      // O log continua a ser atualizado em segundo plano
      ...
      // --== ESSENCIAL: Limpa o temporizador ao final do processo ==--
      clearInterval(progressInterval);
      setIsGenerating(false);
    }
  }

/*--------------- oculto--------16-11-2025 jr
  async function handleGeneratePdf() {
    if (!book?.id) {
      setError("ID do livro não encontrado. Não é possível gerar o PDF.");
      return;
...
// SUBSTITUA SEU RETURN ATUAL POR ESTE BLOCO COMPLETO
  ...
  <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4">
    <Button
      ...
      isLoading={isGenerating} // Mantemos isso para o caso de o componente não desmontar a tempo
