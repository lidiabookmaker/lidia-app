 ...
 * This function is the "digital diagrammer," responsible for applying the correct
 * HTML structure and CSS classes to each part of the book.
 * @param book - The main book object containing metadata like title and author.
 * @param parts - An array of all book parts fetched from the database.
 * @returns A single string containing the complete HTML document for the book.
 */
export const assembleFullHtml = (book: Book, parts: BookPart[]): string => {
  const styles = `
    <style>
        ...
        .page-container { width: 14.8cm; height: 21cm; position: relative; box-sizing: border-box; page-break-after: always; background: white; overflow: hidden; display: flex; flex-direction: column; }
        ...
        .content-page { box-sizing: border-box; padding: 2.4cm 2cm 2.7cm 2cm; }
        ...
        .content-page p { line-height: 15.02pt; margin: 0 0 15.02pt 0; text-align: justify; }
  ...
  parts.sort((a, b) => a.part_index - b.part_index).forEach(part => {
      // Safely parse content, providing a fallback for malformed JSON
      let content;
      try {
        content = JSON.parse(part.content);
      } catch (e) {
        console.error(`Failed to parse content for part_id: ${part.id}`, part.content);
        content = part.content; // Use raw content as a fallback
      }
      
      switch (part.part_type) {
          case 'cover':
              htmlContent += `<div class="page-container cover-page"><div class="content-wrapper"><h1 class="title">${content.title}</h1><p class="subtitle">${content.subtitle}</p><p class="author">${content.author}</p></div><div class="onda onda1"></div><div class="onda onda2"></div><div class="onda onda3"></div></div>`;
              break;
          case 'copyright':
              const copyrightText = typeof content === 'string' ? content : `Copyright © ${new Date().getFullYear()} ${book.author}`;
              htmlContent += `<div class="page-container copyright-page"><div class="content"><p>${copyrightText}</p><p>Todos os direitos reservados.</p><p>Este livro ou qualquer parte dele não pode ser reproduzido ou usado de forma alguma sem a permissão expressa por escrito do editor, exceto pelo uso de breves citações em uma resenha do livro.</p></div></div>`;
              break;
          case 'toc':
               htmlContent += `<div class="page-container content-page"><h1>${content.title}</h1>${content.content.split('\n').map((line: string) => { line = line.trim(); if (!line) return ''; if (line.match(/^capítulo \\d+:/i)) { return `<p class="toc-item toc-chapter">${line}</p>`; } return `<p class="toc-item toc-subchapter">${line}</p>`; }).join('')}</div>`;
              break;
          case 'introduction':
          case 'conclusion':
               htmlContent += `<div class="page-container content-page"><h1>${content.title}</h1>${formatContentForHTML(content.content, true)}</div>`;
              break;
          case 'chapter_title':
              htmlContent += `<div class="page-container chapter-title-page"><h1>${content.title}</h1></div>`;
              break;
          case 'chapter_content':
              // The entire chapter object is stored here
              htmlContent += `<div class="page-container content-page"><h2 class="font-merriweather">${content.title}</h2>${formatContentForHTML(content.introduction, false)}${content.subchapters.map((sub: any) => `<h3 class="font-merriweather-sans">${sub.title}</h3>${formatContentForHTML(sub.content)}`).join('')}</div>`;
              break;
      }
  });

  return `<html><head><title>${book.title}</title>${styles}</head><body>${htmlContent}</body></html>`;
};
